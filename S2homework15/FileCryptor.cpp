...
void FileCryptor::cryptDirectory(const fs::path& dir) 
{
    if (!is_directory(dir)) 
    {
        return;
...
void FileCryptor::decryptDirectory(const fs::path& dir) 
{
    if (!is_directory(dir)) 
    {
        return;
...
void FileCryptor::crypt(const fs::path& file) 
{
...
void FileCryptor::decrypt(fs::path cryptedFile) 
{
    if (!is_regular_file(cryptedFile)) 
    {
        return;
    }
    std::ifstream inputFile(cryptedFile);
